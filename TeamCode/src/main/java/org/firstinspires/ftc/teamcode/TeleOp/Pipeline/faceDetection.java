...
public class faceDetection implements VisionProcessor {
    int wh, ht, index;
    Rect ROI;
    ...
    public void init(int width, int height, CameraCalibration calibration) {
        wh = width;
        ht = height;
        ROI = new Rect(new Point(Math.round((float) wh /10), Math.round((float) ht /10)), new Point(Math.round(0.9*(float) wh), Math.round(0.9*(float) ht)));
    }
    @Override
    public Object processFrame(Mat frame, long captureTimeNanos) {
        detectLargestContours(isolateImage(frame, new Scalar(5,100,100), new Scalar(13, 255, 255), false));
        return null;
    }
    ...
    public void onDrawFrame(Canvas canvas, int onscreenWidth, int onscreenHeight, float scaleBmpPxToCanvasPx, float scaleCanvasDensity, Object userContext) {
        ...
        if (largestContour != null) {
            ...
            canvas.drawText("Face Detected", Math.round((largestcountourrect.x + (float) largestcountourrect.width /2) * scaleBmpPxToCanvasPx) - 40, Math.round((largestcountourrect.y + (float) largestcountourrect.height /2) * scaleBmpPxToCanvasPx), text);
